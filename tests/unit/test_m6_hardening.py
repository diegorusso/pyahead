...
def test_install_smoke_compares_canonical_installed_origin(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Platform aliases such as macOS /var and /private/var remain in-tree."""
    real_root = tmp_path / "real"
    environment = real_root / "environment"
    installed = environment / "lib" / "pyahead" / "__init__.py"
...
def test_false_positive_sample_is_deterministic_and_bounded() -> None:
    ...
    first = corpus._worksheet_rows([repository], sample_size=_SAMPLE_SIZE)
    second = corpus._worksheet_rows([repository], sample_size=_SAMPLE_SIZE)
...
def test_locked_dev_environment_has_supported_host_rpds_wheels() -> None:
    """Schema-test dependencies install without an unsupported source build."""
    ...
    rpds = next(package for package in packages if package["name"] == "rpds-py")
    wheels = cast("list[dict[str, str]]", rpds["wheels"])
    urls = tuple(wheel["url"] for wheel in wheels)
    ...
    assert all(any(tag in url for url in urls) for tag in required_tags)
