...
def _verify_worksheet_identity(result_path: Path, worksheet_path: Path) -> None:
    ...
    with worksheet_path.open(encoding="utf-8", newline="") as stream:
        ...
        if tuple(reader.fieldnames or ()) != _WORKSHEET_FIELDS:
            ...
            raise CorpusError(message)
        rows = list(reader)
    if not rows or rows[0]["row_kind"] != "corpus-identity":
        message = "worksheet is missing its corpus identity row"
        raise CorpusError(message)
    if rows[0]["corpus_result_sha256"] != expected:
        message = "worksheet corpus identity does not match the result"
        raise CorpusError(message)
    for row in rows[1:]:
...
def main(argv: list[str] | None = None) -> int:
    """Scan a corpus or verify the identity binding of existing review files."""
    arguments = _parser().parse_args(argv)
    try:
        if arguments.verify_identity:
            _verify_worksheet_identity(arguments.output, arguments.worksheet)
            return 0
        manifest = _require_manifest(arguments.manifest).resolve(strict=True)
        ...
        result_text = json.dumps(document, indent=2, sort_keys=True) + "\n"
