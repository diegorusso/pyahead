...
The process stops without replacing either output if a checkout is dirty,
mis-pinned, a scan fails, output is malformed, or a report exceeds 64 MiB. It
uses isolated Python module lookup so a target checkout cannot replace the
installed `pyahead` module.
...
without hand-picking favorable results. If fewer findings exist, review all of
them. If no high-confidence finding exists, no precision claim is possible.

Start from [`false-positive-review.csv`](false-positive-review.csv) or the
generated worksheet. For every row, inspect the pinned source and rule evidence,
then set `classification` to exactly one of:
